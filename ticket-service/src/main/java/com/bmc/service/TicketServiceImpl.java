...
public class TicketServiceImpl implements TicketService {
    ...
    public TicketResponseDto bookTicketWithDiscount(TicketRequestDto ticketRequestDto) throws NoUserFoundException, ShowNotFound, Exception {
        ...
        int totalPrice = calculatePriceWithDiscount( show, ticketRequestDto);
        ticket.setTotalTicketsPrice(totalPrice);
    ...
    private boolean isAfternoonShow(String showTime) {
        LocalTime showStart = LocalTime.parse(showTime);
        LocalTime noon = LocalTime.of(12, 0);
        LocalTime evening = LocalTime.of(17, 0);
        return showStart.isAfter(noon) && showStart.isBefore(evening);
    }

    private double calculatePriceWithDiscount(Show show, TicketRequestDto request) {
        double ticketPrice = request.getTicketPrice();
        int numTickets = request.getTickets().size();
        double totalPrice = 0.0;

        // Apply 50% discount on the third ticket
        for (int i = 0; i < numTickets; i++) {
            if (i == 2) {
                totalPrice += 0.5 * ticketPrice;  // 50% discount on the third ticket
            } else {
        ...
        if (isAfternoonShow(showDetails.getShowTime())) {
            totalPrice *= 0.8;  // 20% discount for afternoon shows
        }
